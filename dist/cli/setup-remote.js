...
import { CONFIG_DIR, REMOTE_KEYS_DIR, PEER_KEYS_DIR, REMOTE_CONFIG_PATH, saveRemoteConfig, } from '../shared/config.js';
...
`);
        ...
        if (importPath) {
            ...
            console.log(`  public key files to: ${path.join(PEER_KEYS_DIR, 'authorized-clients', '<client-name>')}`);
        }
        ...
        while (addingRoutes) {
            ...
            while (addingSecrets) {
                ...
                if (!name) {
                    ...
                    const value = await ask(rl, `    Value for "${name}" (use \${VAR} for env vars)`, `\${${name}}`);
                    secrets[name] = value;
                }
            }
            const route = {
            ...
            if (routeDocsUrl)
                route.docsUrl = routeDocsUrl;
            if (routeOpenApiUrl)
                route.openApiUrl = routeOpenApiUrl;
        ...
        console.log(`     copy them to: ${path.join(PEER_KEYS_DIR, 'authorized-clients', '<client-name>')}`);
...
//# sourceMappingURL=setup-remote.js.map