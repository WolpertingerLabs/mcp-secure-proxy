...
import { HandshakeInitiator, HandshakeResponder } from './handshake.js';
...
describe('HandshakeResponder', () => {
    ...
    it('should reject unsupported versions', () => {
        ...
        const init = initiator.createInit();
        // Change version
        const badVersion = { ...init, version: 2 };
        const responder = new HandshakeResponder(responderKeys, [initiatorPub]);
...
describe('Full handshake flow', () => {
    ...
    it('should produce complementary send/recv keys', () => {
        ...
        expect(result.initiatorSessionKeys.sendKey.encryptionKey.equals(result.responderSessionKeys.recvKey.encryptionKey)).toBe(true);
        // Initiator's recvKey == Responder's sendKey
        expect(result.initiatorSessionKeys.recvKey.encryptionKey.equals(result.responderSessionKeys.sendKey.encryptionKey)).toBe(true);
    });
