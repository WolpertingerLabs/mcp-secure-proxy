...
async function sendEncryptedRequest(toolName, toolInput) {
    ...
    const response = ch.decryptJSON(encryptedResponse);
    if (!response.success) {
 ...
 */
server.tool('secure_request', 'Make an authenticated HTTP request through the encrypted proxy. Use ${VAR_NAME} placeholders for secrets — they are resolved server-side and never exposed to the client.', {
 ...
 */
server.tool('get_secret', 'Retrieve a secret value by name from the secure remote store. The value is encrypted in transit and never stored locally.', {
 ...
 */
server.tool('list_secrets', 'List the names of available secrets on the remote server. Does not return secret values.', 
// Empty schema — no input needed
{ _: z.string().optional().describe('unused') }, async () => {
    try {
